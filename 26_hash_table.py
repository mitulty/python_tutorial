...
# @Last Modified time: 2024-12-15 15:21:22
'''
- Arrays and Linked Lists have time complexity O(n) for search and delete, while Hash Tables have just O(1) on average! 
- Hash Table elements are stored in storage containers called buckets. Every Hash Table element has a part that is unique and is called the key.
...
- Open Addressing is another way to solve collisions. With open addressing, if there is a need to store an element but there is already an element in that 
